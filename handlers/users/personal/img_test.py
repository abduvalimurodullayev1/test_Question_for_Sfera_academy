...
from loader import dp, bot, db5
from utils.db_api.test_db import Image_test, Category_2, TestResult, Test_Savol
...
async def user_start_test(message: types.Message):
    tests = db5.all_imagetest()
    for test in tests:
        sent_photo = await message.answer_photo(photo=test.test_image)
        ...
        photo_to_message[sent_poll.poll.id] = sent_photo.message_id
    return sent_poll


@dp.message_handler(Text(equals="👁 Rasmli testlar 🖼"))
async def select_category_handler(message: types.Message):
    categories = Category_2.all_categories()
    if not categories:
...
@dp.message_handler(lambda message: any(category.title == message.text for category in Category_2.all_categories()))
async def start_tests_handler(message: types.Message):
    category = next((cat for cat in Category_2.all_categories() if cat.title == message.text), None)
    if not category:
        await message.answer("Kategoriya topilmadi.")
        return

    tests = Image_test.get_tests_by_category(category.id)
    if not tests:
