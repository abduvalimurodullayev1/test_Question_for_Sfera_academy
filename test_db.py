...
class User(Base):
    ...
    test_result = relationship("TestResult", back_populates='user')
...
@classmethod
def add_register(cls, full_name, phone_number, region, school, class_s):
...
@classmethod
def all_register(cls):
    with get_db() as session:
        try:
            return session.query(cls).all()
        except SQLAlchemyError as e:
            session.rollback()
            print(f"Error fetching all registers: {e}")
            return []
...
class Category(Base):
    ...
    tests = relationship("TestQuestion", back_populates="category")
...
class TestQuestion(Base):
    ...
    def all_Test(self):
        tests = session.query(TestQuestion).all()
        return tests

    def delete_test_savol(self, test_id):
        test = session.query(TestQuestion).get(test_id)
        session.delete(test)
        session.commit()
        return test
    ...
    def add_test(cls, category, question, a_variant, b_variant, c_variant, correct_answer):
        with get_db() as session:
            try:
                ...
                session.rollback()
                print(f"Error adding TestQuestion: {e}")
                return None
...
class ImageTest(Base):
    ...
    def add_imagetest(self, category_id, test_image, question, a_variant, b_variant, c_variant, correct_answer):
        test = ImageTest(category_id=category_id, test_image=test_image, question=question, a_variant=a_variant,
                         b_variant=b_variant, c_variant=c_variant, correct_answer=correct_answer)
        session.add(test)
        session.commit()
        return test
    ...
    def all_imagetest(self):
        test = session.query(ImageTest).all()
        return test


def delete_test_question(self, test_id, session):
    test = session.query(ImageTest).get(test_id)
    session.delete(test)
    session.commit()
    return test
